			 ...
			 umode_t mode, bool want_excl)
{
	...
	saved_cred = override_fsids(SDCARDFS_SB(dir->i_sb)->options.fs_low_uid,
					SDCARDFS_SB(dir->i_sb)->options.fs_low_gid);
	if (!saved_cred)
		return -ENOMEM;
			 ...
			 struct inode *new_dir, struct dentry *new_dentry)
{
	...
	/* save current_cred and override it */
	saved_cred = override_fsids(SDCARDFS_SB(old_dir->i_sb)->options.fs_low_uid,
					SDCARDFS_SB(old_dir->i_sb)->options.fs_low_gid);
	if (!saved_cred)
		return -ENOMEM;
...
static int sdcardfs_setattr(struct dentry *dentry, struct iattr *ia)
{
	...
	err = notify_change(lower_dentry, &lower_ia, &lower_dentry->d_inode); /* note: lower_ia */
